...
public class OAuthRegistry {
	...
	static { // We load the configuration of the OAuth providers when the class is loaded by
				// the server:
		loadConfig();
	}
	 ...
	 * @throws IOException
	 */
	public static AuthorizationCodeFlow initializeFlow(String tokenURL, String clientID, String clientSecret,
			String authorizationFormURL, Set<String> scopes) throws IOException {
		AuthorizationCodeFlow.Builder flowBuilder = new AuthorizationCodeFlow.Builder(
				...
				new GenericUrl(tokenURL), new ClientParametersAuthentication(clientID, clientSecret), clientID,
				authorizationFormURL).setDataStoreFactory(MemoryDataStoreFactory.getDefaultInstance());
	...
	public static AuthorizationCodeFlow initializeFlow(String service) throws IOException {
		...
		return initializeFlow(config.getTokenUrl(), config.getClientId(), config.getClientSecret(),
				config.getAuthorizationFormUrl(), config.getScopes());
	}

	public static void onAuthorizationSuccess(String onSuccess, String provider, Credential credential,
		...
		if (onSuccess != null) {
			...
			response.getWriter().append("Access token for ").append(provider + ": ")
	...
	private static void loadConfig() {
		...
		try {
			...
			if (is == null)
				...
				serviceConfigurations = (Map<String, OAuthServiceConfiguration>) om.readValue(is,
						new TypeReference<HashMap<String, OAuthServiceConfiguration>>() {
						});
			}
